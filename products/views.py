...
def products_view(request):
    ...
    return render(request,'products/productspage.html',{'products':products})
...
def create_view(request):
    if request.method == 'POST':
        if request.POST['title'] and request.POST['body'] and request.POST['url'] and request.FILES['icon'] and request.FILES['image']:
            ...
            return render(request,'products/create.html',{'error':'Insufficient Fields'})  
    else:     
...
@login_required(login_url="/account/signup")
def upvote_view(request,product_id):
    if request.method == "POST":
